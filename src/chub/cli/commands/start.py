"""``chub start`` — one-command bootstrap (daemon + hooks + tui).

Behaviour:
  1. Ensure chubd is running (start it detached if not).
  ...
  4. Optionally bulk-register every promote_required candidate as a readonly
     session (``--auto-attach``).
  5. Unless ``--no-tui`` is set, ``os.execv`` into the chub-tui binary.
"""
...
def run(
    ...
    from chub.cli.commands import tui as tui_mod
