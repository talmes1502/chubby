"""`chub diag <name>` — print captured wrapper stderr for a session.
...
Sessions that were registered outside of ``chub spawn`` (raw user-launched
``chub-claude`` etc.) won't have a diagnostic file — we tell the user that
...
def run(
    ...
    async def go() -> None:
        ...
        if not log_path.exists():
            ...
            typer.echo(
                "this session was probably wrapper-launched outside `chub spawn` "
                "(e.g., user-launched chub-claude). diagnostics only capture "
