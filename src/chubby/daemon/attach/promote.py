"""Promote-attach: wait for raw claude PID to exit, then spawn chubby-claude.
...
``chubby-claude`` so it joins the registry as a wrapped session.
"""
...
async def relaunch_wrapper(*, name: str, cwd: str, tags: list[str]) -> int:
    """Spawn a detached ``chubby-claude`` wrapper that will register itself."""
