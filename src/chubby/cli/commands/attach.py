"""``chubby attach`` — discover running ``claude`` processes and attach to them.

Modes:
- ``chubby attach --list``               : print candidates as JSON
- ``chubby attach --pick``               : interactive picker
- ``chubby attach --all``                : attach to every tmux-attachable candidate
- ``chubby attach --as-readonly``        : bulk-register every promote_required
                                          candidate as a readonly session
- ``chubby attach tmux:session:0.0``     : attach to a specific tmux pane
"""
