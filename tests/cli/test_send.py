"""End-to-end test for `chub send` — wires Typer command into a real Server."""
...
from chub.cli.commands import send as send_cmd
...
async def test_send_resolves_name_and_calls_inject(
    ...
    # ``send`` resolves the daemon socket through ``chub.daemon.paths.sock_path``;
    # patch that for the duration of this test so the CLI hits our fake server.
    monkeypatch.setattr("chub.daemon.paths.sock_path", lambda: sock)
